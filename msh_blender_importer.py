...
def find_texture(texture_filepath, search_directories, acceptable_extensions, recursive=False, auto_convert_dxtbz2=False):
	...
	# Optional: auto-convert BZCC .dxtbz2 textures to .dds
	if auto_convert_dxtbz2 and original_extension_compare == ".dxtbz2":
		# Try to locate the source .dxtbz2 on disk
		source_path = texture_filepath
		if not os.path.exists(source_path):
			for directory in search_directories:
				for root, folders, files in os.walk(directory):
					path = os.path.join(root, file_name + original_extension)
					if PRINT_TEXTURE_FINDER_INFO:
						print("TEXTURE FINDER %r:" % file_name, "Checking for DXTBZ2", path)
		...
		if os.path.exists(source_path):
			dds_path = os.path.splitext(source_path)[0] + ".dds"
			try:
				if not os.path.exists(dds_path):
					print("BZ2 MSH: Converting %r -> %r via undxtbz2" % (source_path, dds_path))
					undxtbz2.dxtbz2_to_dds(source_path, dds_path)
				else:
					print("BZ2 MSH: Using existing DDS %r for %r" % (dds_path, source_path))
				
				if os.path.exists(dds_path):
					if PRINT_TEXTURE_FINDER_INFO:
						print("TEXTURE FINDER %r: Auto-converted dxtbz2 to %r" % (file_name, dds_path))
					return dds_path
			except Exception as exc:
				print("BZ2 MSH: Failed to convert %r: %s" % (source_path, exc))
...
class Load:
	...
	def create_material(self, msh_material=None, msh_texture=None):
		...
		if image_is_material_file:
			...
			material_filepath = find_texture(material_name, find_in, self.ext_list, recursive, auto_convert_dxtbz2=auto_convert_dxtbz2)
			if os.path.exists(material_filepath):
				texture_names = read_material_file(material_filepath, default_diffuse=texture_name)
				texture_paths = {which: find_texture(name, find_in, self.ext_list, recursive, auto_convert_dxtbz2=auto_convert_dxtbz2) for (which, name) in texture_names.items() if name}
			...
			image_filepath = find_texture(texture_name, find_in, self.ext_list, recursive, auto_convert_dxtbz2=auto_convert_dxtbz2)
			bpy_node_texture = bpy_material.node_tree.nodes.new("ShaderNodeTexImage")
